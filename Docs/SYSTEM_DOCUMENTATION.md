...
- **Model**: Hybrid (Linear trend + Prophet with daily + weekly seasonality)
- **Output**: Days until disk reaches 90% capacity
- **Horizon**: 7 days (configurable via `horizon_days` parameter)
- **Storage**: Manifest-based (`disk_full_models.pkl`)
- **Features**:
  - Per-node, per-mountpoint predictions
  - **Temporal-Aware**: Prophet learns daily/weekly patterns (batch jobs, weekend backups)
  - Severity classification (CRITICAL, WARNING, SOON, OK)
  - CSV report generation
  - Horizon displayed in all outputs and plot titles

#### I/O and Network Crisis Prediction
- **Input**: Disk I/O wait and Network transmit bandwidth
- **Model**: Prophet with daily + weekly seasonality (crisis detection)
...
- **Temporal-Aware**: Learns daily/weekly patterns to reduce false positives

#### I/O and Network Ensemble Forecast
- **Input**: Same as crisis prediction
- **Model**: Full ensemble (Prophet + ARIMA + LSTM)
- **Output**: Detailed forecasts with temporal-aware anomaly detection
- **Storage**: Manifest-based (`io_net_models.pkl`)
- **Temporal-Aware Anomaly Detection**:
  ...
  - Auto-enabled when 3+ months of historical data available
...
- Plot generation is optional: use `--plot <forecast|backtest|both>` flag to generate PNG files. Options:
  - `--plot forecast`: Generate only forecast plots (future predictions)
  - `--plot backtest`: Generate only backtest plots (model performance evaluation)
  - `--plot both`: Generate both forecast and backtest plots
  - Without `--plot`, plots are skipped for faster execution.
...
- Forecast plots: `*_forecast.png` (only when `--plot forecast` or `--plot both` is provided)
- Backtest plots: `*_backtest.png` (only when `--plot backtest` or `--plot both` is provided)
...
**Note:** Plot generation can be time-consuming. Use `--plot` only when visualizations are needed. For production monitoring focused on alerts, omit `--plot` for faster execution. In training mode (`--training`), plots are generated by default (both forecast and backtest).
...
**Generated When**:
- `--training` mode (generated by default)
- `--forecast --plot forecast` or `--forecast --plot both`
- During retraining with plot flags enabled

**Not Generated When**:
- Normal mode (no flags)
- `--forecast` without `--plot` flag
- `--forecast --plot backtest` (only backtest plots generated)
...
- `--training` mode (generated by default)
- `--show-backtest` mode
- `--forecast --plot backtest` or `--forecast --plot both`
- During retraining with plot flags enabled (with `--disk-retrain` or `--io-net-retrain`)

**Not Generated When**:
- `--forecast` without `--plot` flag
- `--forecast --plot forecast` (only forecast plots generated)
- Normal mode (no flags)
...
**Implementation**:
- **Training Mode**: Generates both forecast and backtest plots by default
- **Forecast Mode**: Plot generation controlled by `--plot` flag:
  - `--plot forecast`: Generate only forecast plots
  - `--plot backtest`: Generate only backtest plots
  - `--plot both`: Generate both forecast and backtest plots
  - Without `--plot`: No plots generated (optimized for speed)
- **Show Backtest Mode**: Generates backtest plots only
- **Normal Mode**: No plots generated
- **Retrain Mode**: Generates plots for retrained models when plot flags are enabled
...
#### CLI Overrides
- `--plot <forecast|backtest|both>`: Generate and save plot files (PNG images). Options:
  - `forecast`: Generate only forecast plots (future predictions)
  - `backtest`: Generate only backtest plots (model performance evaluation)
  - `both`: Generate both forecast and backtest plots
  - If not specified, plots are skipped to save time.
...
- `--forecast-horizon <realtime|neartime|future>`: Override `HORIZON_MIN` for forecast length:
  - `realtime`: 15 minutes (short-term decisions)
  - `neartime`: 3 hours (near-term planning)
  - `future`: 7 days (long-term capacity planning)
  - Default: Uses `HORIZON_MIN` env var (15 minutes)
  - Horizon is displayed in all outputs and plot titles for transparency
