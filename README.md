...
python3 metrics.py --forecast --interval 15 --plot both
```
...
- Generates forecast plots for all metrics (only when `--plot forecast` or `--plot both` is provided)
- Generates backtest plots for all models (only when `--plot backtest` or `--plot both` is provided)
